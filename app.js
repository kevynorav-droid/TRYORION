...
window.respaldoResultadosHTML = ""; // <--- Aquí guardaremos tu lista estilo Google antes de que se borre
const GROQ_API_KEY = "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"; // Reemplaza aquí con tu API Key segura cuando la uses
...
clearHistoryBtn.addEventListener('click', async () => {
  ...
  try {
    // 1. Verificamos si el dispositivo soporta biometría local (Face ID / Touch ID)
    const disponible = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
    if (!disponible) {
      alert("Face ID / Touch ID no están disponibles o configurados en este dispositivo.");
      return;
    }

    // Buscamos si ya registramos una llave local para el historial en este iPhone
    let credentialIdRaw = localStorage.getItem('orion_biometric_id');

    if (!credentialIdRaw) {
      // === PASO A: REGISTRO (Solo se ejecuta la primerísima vez) ===
      const credential = await navigator.credentials.create({
        publicKey: {
          challenge: crypto.getRandomValues(new Uint8Array(32)),
          rp: { name: "ORION App", id: window.location.hostname },
          user: {
            id: crypto.getRandomValues(new Uint8Array(16)),
            name: "usuario_orion",
            displayName: "Usuario Orion"
          },
          pubKeyCredParams: [{ type: "public-key", alg: -7 }], // ES256 nativo de Apple
          authenticatorSelection: {
            authenticatorAttachment: "platform", // Obliga a usar FaceID/TouchID interno
      ...
      if (credential) {
        // Guardamos la referencia de la llave para las siguientes consultas
        const idBase64 = btoa(String.fromCharCode(...new Uint8Array(credential.rawId)));
        localStorage.setItem('orion_biometric_id', idBase64);
        abrirModalHistorial();
      }
    } else {
      // === PASO B: VERIFICACIÓN (Se ejecuta siempre a partir del segundo clic) ===
    ...
    alert("❌ Acceso denegado. Se requiere autenticación para ver el historial.");
...
async function preguntarOrionAI(texto, esModoMaths = false) {
  try {
    ...
    if (data.error) {
      return `⚠️ ERROR DE GROQ: ${data.error.message}`;
    }
...
window.verArticuloCompleto = async function(title, lang, imgUrl) {
  // LE TOMAMOS UNA FOTO A LA LISTA DE RESULTADOS ANTES DE QUE SE BORRE DE LA PANTALLA
...
// Reasigna los clics nativos a las tarjetas cuando se restaura el HTML guardado
