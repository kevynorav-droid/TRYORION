...
clearHistoryBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!credentialIdRaw) {
      // === PASO A: REGISTRO (Primer clic en tu iPhone) ===
      const credential = await navigator.credentials.create({
        publicKey: {
          challenge: crypto.getRandomValues(new Uint8Array(32)),
          rp: { 
            name: "ORION App", 
            id: "kevynorav-droid.github.io" // Tu dominio fijo de GitHub
          },
          user: {
      ...
      const assertion = await navigator.credentials.get({
        publicKey: {
          challenge: crypto.getRandomValues(new Uint8Array(32)),
          rpId: "kevynorav-droid.github.io", // Tu dominio fijo de GitHub
          allowCredentials: [{
            type: "public-key",
            id: idBytes
          }],
